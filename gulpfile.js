...
gulp.task('compress-js',function (callback) {       //- 创建一个名为compress-js的task
    gulp.src(['fleet-footed-customer/js2/**/*.js'])             //- 需要处理的js文件，放到一个字符串数组里
        .pipe(uglify())                             //- 压缩文件
        .pipe(rev())                                //- 文件名加MD5后缀
        .pipe(gulp.dest('dist/js2'))                 //- 另存压缩后的文件
...
gulp.task('compress-css', function(callback) {      //- 创建一个名为compress-css的task
    gulp.src(['fleet-footed-customer/css2/**/*.css'])           //- 需要处理的css文件，放到一个字符串数组里
        // .pipe(concat('css/wap.min.css'))         //- 合并后的文件名
        .pipe(minifyCss())                          //- 压缩处理成一行
        .pipe(rev())                                //- 文件名加MD5后缀
        .pipe(gulp.dest('dist/css2'))                //- 输出文件到dist/css目录下
        .pipe(rev.manifest())                       //- 生成一个rev-manifest.json
...
gulp.task('rev-html',['compress-css','compress-js'], function() {          //- compress-css和compress-js任务执行完毕再执行rev-index任务
    ...
    gulp.src(['rev-css/*.json','rev-js/*.json', 'fleet-footed-customer/**/*.html'])   //- 读取两个rev-manifest.json文件以及需要进行css和js名替换的html文件
    ...
    gulp.src(['rev-css/*.json','rev-js/*.json', 'fleet-footed-customer/pages2/**/*.html'])     //- 读取两个rev-manifest.json文件以及需要进行css和js名替换的html文件
        .pipe(revCollector())                                                      //- 执行文件内css和js名的替换
        .pipe(gulp.dest('dist/pages2'));                                            //- 替换后的html文件输出的目录
});

/*压缩并复制图片*/
gulp.task('compress-img',function () {
    gulp.src('fleet-footed-customer/img2/**/*.*')    //原图片的位置
        .pipe(imagemin())                   //执行图片压缩
        .pipe(gulp.dest('dist/img2'));    //压缩后的图片输出的位置
});
